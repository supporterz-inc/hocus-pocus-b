...
これは、3 Days インターンシップで形成されるチームの性質に応じて、 実装に特色が出ていくことを期待しているからです。
...
Git および GitHub を用いた開発の流れは、以下を参考にしてください：

```mermaid
sequenceDiagram
    box Git の操作
    participant main as `main`
    participant dev as `dev` (例)
    end
    participant github as GitHub

    main->>dev: git switch -c dev

    activate dev
    Note over dev: 開発作業<br>(※ 細かい区切りが望ましい)
    Note over dev: git add '編集したファイル名' ...
    Note over dev: git commit -m '開発作業の要約'
    dev-->>dev: 開発作業の分だけ繰り返し ...
    dev->>github: git push origin dev
    deactivate dev

    activate github
    Note over github: Pull-Request の作成
    github-->>dev: 開発作業の不備などを指摘
    activate dev
    Note over dev: 修正作業
    dev->>github: git push origin dev
    deactivate dev
    Note over github: GitHub 上の `main` へ Merge
    github-->>dev:
    deactivate github
    ...
    activate main
    github->>main: git pull main
    deactivate main
```

なお、開発用の **GitHub** Repository には、以下の制限がかけられています：
...
[`CHALLENGE.md`](CHALLENGE.md) を参照しながら、チームで協力してベースアプリケーションのスコープを実装してください。
