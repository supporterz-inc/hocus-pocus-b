...
const iapIssuer = 'https://edu-functor.cloudflareaccess.com';
const iapAudience = process.env['IAP_AUDIENCE']!;
const jwks = createRemoteJWKSet(new URL(`${iapIssuer}/cdn-cgi/access/certs`));
const port = parseInt(process.env['PORT'] ?? '8080', 10);

async function getUserName(cookie: string | undefined): Promise<string> {
  if (cookie == null) return 'Unknown User';

  const response = await fetch(`${iapIssuer}/cdn-cgi/access/get-identity`, { headers: { Cookie: cookie } });
...
app.use(
  '/index.css',
  serveStatic({
    path: 'target/index.css',
    onFound: (_, ctx) => ctx.header('Cache-Control', 'no-cache'),
  }),
);
